...
function toolsSchema(value) {
  const v = (value && typeof value === 'object') ? value : {}
  return { monthlyAllowance: normalizeAllowance(v.monthlyAllowance) }
}
...
export function apply(ctx) {
  ...
  function readToolsConfig() {
    ...
    const obj = (v && typeof v === 'object') ? v : {}
    return { monthlyAllowance: normalizeAllowance(obj.monthlyAllowance) }
  }
  ...
  registerRoute('/ollama', 'usage', async () => {
    ...
    const configured = allowance > 0
    return {
      ok: true,
      usage: {
        fraction,
        // 未設定額度時回 null，讓 UI 明確顯示「等待填入」，而不是編一個數字出來。
        allowance: configured ? allowance : null,
        usedDollars: (fraction !== null && configured) ? fraction * allowance : null,
      },
    }
  })
