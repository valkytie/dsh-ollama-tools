...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 星期索引：1=週一 … 7=週日（Date.getDay() 是 0=週日，這裡統一成 1..7）。
    ...
    // 取「指定時區」的現在時刻。分鐘數與星期必須來自同一個時區，
    // 否則瀏覽器時區與設定時區不同時，跨日邊界會判錯尖峰/離峰。
    function zonedClock(date, timezone) {
      const localDay = date.getDay()
      const local = { minutes: date.getHours() * 60 + date.getMinutes(), weekday: localDay === 0 ? 7 : localDay }
      ...
      try {
        ...
        for (const part of zonedFormatter(timezone).formatToParts(date)) {
          ...
          else if (part.type === 'minute') mm = parseInt(part.value, 10)
          else if (part.type === 'weekday') weekday = WEEKDAY_INDEX[part.value]
        }
        if (hh !== null && mm !== null && weekday !== undefined) return { minutes: hh * 60 + mm, weekday }
      } catch (e) { /* 無效時區：退回本機時間 */ }
      return local
    }
    ...
    // 找下一個狀態轉換：沿著「設定時區」的本地時鐘逐分推進，跨午夜時星期一起進位，
    // 週末與跨日邊界才不會算錯。最多掃 7 天，找不到就回 null。
    // 註：不處理 DST 跳躍（Asia/Taipei 無日光節約時間，影響可忽略）。
