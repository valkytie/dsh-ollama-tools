    ...
    function createDualPeakWidget(ctx) {
      return function DualPeakWidget() {
        ...
        const refreshUsage = React.useCallback(() => {
          ...
          apiCall('/ollama', 'usage').then((res) => {
            if (res && res.ok) {
              ...
              setUsage(null)
              setUsageError((res && res.error) || '查詢失敗')
            }
          }).catch((err) => {
            setUsage(null)
            setUsageError(String(err && err.message ? err.message : err))
          }).finally(() => {
        ...
        const saveSettings = () => {
          ...
          setLoading(true)
          apiCall('/dualpeak', 'setConfig', patch).then((res) => {
            if (res && res.ok) {
              setCfg(res.config)
              setError(null)
        ...
        if (usageLoading) {
          ...
          const limits = usage.limits && usage.limits.monthly
          const activity = usage.activity || {}
          const models = (limits && limits.models) || []
          const rows = models.map((m) => React.createElement('div', { className: 'dpk-provider', key: m.name },
            React.createElement('span', { className: 'dpk-dot offpeak' }),
            React.createElement('span', { className: 'dpk-pname' }, m.name),
            React.createElement('span', { className: 'dpk-pstate' }, String(m.request_count) + ' 次'),
          ))
          usageSection = React.createElement('div', { className: 'dpk-body' },
            React.createElement('div', { className: 'dpk-reco' },
              'Ollama 本月用量：' + (limits ? String(limits.usage) : '?') + ' · 成本：$' + (activity.cost || '0.00'),
            ),
            rows.length > 0 ? rows : React.createElement('div', { className: 'dpk-meta' }, '本月尚無模型用量'),
            React.createElement('div', { className: 'dpk-meta' },
              '期間：' + ((activity.period && activity.period.starting_at || '').slice(0, 10)) + ' ~ ' + ((activity.period && activity.period.ending_at || '').slice(0, 10)),
            ),
          )
        }
