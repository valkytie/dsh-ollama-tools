...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 官方窗都落在同一個 UTC 日內，不需要拆跨午夜；週末（UTC 週六日）自動全離峰。
    // 所以判定一律用 UTC，與「時區」設定無關（那個設定只用來顯示轉換時刻）。
    const PROVIDERS = [
      {
        id: 'deepseek',
        name: 'DeepSeek 官方',
        ...
        note: '尖峰 台北 週一至五 09:00–12:00 / 14:00–18:00（官方 UTC 01:00–04:00 / 06:00–10:00，週一至五；台北週末全日離峰）',
      },
      {
        id: 'ollama',
        name: 'Ollama Cloud',
        ...
        note: '尖峰 台北 週一至五 20:00–24:00 與 週二至週六 00:00–02:00（官方 UTC 12:00–18:00，週一至五）',
      },
    ]

    // 顯示用的中文星期（en-GB 的 short weekday → 中文）。
    const WEEKDAY_LABEL = { Mon: '週一', Tue: '週二', Wed: '週三', Thu: '週四', Fri: '週五', Sat: '週六', Sun: '週日' }
    ...
    function zonedFormatter(timezone) {
      ...
      if (f === undefined) {
        f = new Intl.DateTimeFormat('en-GB', {
          timeZone: timezone, hour: '2-digit', minute: '2-digit', hourCycle: 'h23', weekday: 'short',
        })
    ...
    // 判定用的時鐘：官方定義在 UTC，直接取 UTC 的分鐘數與星期（1=週一 … 7=週日）。
    function utcClock(date) {
      const day = date.getUTCDay()
      return { minutes: date.getUTCHours() * 60 + date.getUTCMinutes(), weekday: day === 0 ? 7 : day }
    }
    ...
    function providerState(clock, prov) {
      if (clock.weekday > 5) return 'offpeak'
      return prov.peakRanges.some((r) => inRange(clock.minutes, r)) ? 'peak' : 'offpeak'
    }

    // 找下一個狀態轉換：沿 UTC 逐分推進，跨午夜時星期一起進位。最多掃 7 天，找不到回 null。
    function nextTransition(clock, prov) {
      const current = providerState(clock, prov)
      ...
      for (let i = 1; i <= 7 * 1440; i++) {
        ...
        const state = providerState({ minutes, weekday }, prov)
        if (state !== current) return { minutes: i, next: state }
      }
      return null
    }

    // 轉換時刻的顯示文字（純顯示，不影響判定）。
    function transitionLabel(minutes, timezone) {
      const at = new Date(Date.now() + minutes * 60000)
      const names = ['週日', '週一', '週二', '週三', '週四', '週五', '週六']
      const localLabel = names[at.getDay()] + ' ' + String(at.getHours()).padStart(2, '0') + ':' + String(at.getMinutes()).padStart(2, '0')
      if (timezone) {
        try {
          let wd = null
          let hh = null
          let mm = null
          for (const part of zonedFormatter(timezone).formatToParts(at)) {
            if (part.type === 'weekday') wd = WEEKDAY_LABEL[part.value]
            else if (part.type === 'hour') hh = part.value
            else if (part.type === 'minute') mm = part.value
          }
          if (wd !== undefined && hh !== null && mm !== null) return wd + ' ' + hh + ':' + mm
        } catch (e) { /* 無效時區：退回本機時間 */ }
      }
      return localLabel
    }
...
`
    ...
    function createDualPeakWidget(ctx) {
      return function DualPeakWidget() {
        ...
        const [cfg, setCfg] = React.useState({ timezone: '' })
        ...
        const [usageFetchedAt, setUsageFetchedAt] = React.useState(null)
        const [usageCacheMinutes, setUsageCacheMinutes] = React.useState(null)
        // 未設定額度＝null（等待使用者填入），不再預設任何金額。
        const [allowanceDraft, setAllowanceDraft] = React.useState('')
        const [cacheDraft, setCacheDraft] = React.useState('')

        // 視窗尺寸 + 卡片實測高度：卡片展開用量/設定時會長高，用實測值夾邊界才準。
        ...
        // render 後量測卡片實際高度（同值 setState 時 React 會跳過重繪，不會迴圈）
        React.useEffect(() => {
          const el = cardRef.current
          if (!el) return
          const h = el.getBoundingClientRect().height
          if (h > 0) setCardH(Math.round(h))
        })

        const refreshUsage = React.useCallback((force) => {
          setUsageLoading(true)
          apiCall('/ollama', 'usage', force ? { force: true } : null).then((res) => {
            if (res && res.ok) {
              setUsage(res.usage)
              setUsageError(null)
              setUsageFetchedAt(typeof res.fetchedAt === 'number' ? res.fetchedAt : null)
              const cacheMin = (typeof res.cacheMinutes === 'number') ? res.cacheMinutes : null
              setUsageCacheMinutes(cacheMin)
              if (cacheMin !== null) setCacheDraft(String(cacheMin))
              const stored = (res.usage && typeof res.usage.allowance === 'number' && res.usage.allowance > 0) ? res.usage.allowance : null
              setAllowanceDraft(stored === null ? '' : String(stored))
            } else {
              setUsage(null)
        ...
        const clock = utcClock(new Date())

        const states = PROVIDERS.map((p) => {
          const st = providerState(clock, p)
          const trans = nextTransition(clock, p)
          return { prov: p, state: st, trans, at: trans === null ? '' : transitionLabel(trans.minutes, cfg.timezone) }
        })
        ...
        const saveSettings = () => {
          ...
          const cacheNum = parseInt(cacheDraft, 10)
          // 留空＝清除額度（0），卡片回到只顯示百分比；有填才當成設定值。
          const allowPatch = {
            monthlyAllowance: (isFinite(allowNum) && allowNum > 0) ? allowNum : 0,
            usageCacheMinutes: (isFinite(cacheNum) && cacheNum >= 0) ? cacheNum : 10,
          }
          setLoading(true)
          Promise.all([
            apiCall('/dualpeak', 'setConfig', patch),
            apiCall('/ollama', 'setConfig', allowPatch),
          ]).then(([r1, r2]) => {
        ...
        const providerRow = (s) => {
          // 只顯示狀態，不宣稱倍率：程式裡沒有價格來源，實際折扣以官方公告為準。
          const label = s.state === 'peak' ? '尖峰' : '離峰'
          const count = s.trans
            ? ' · ' + fmtDuration(s.trans.minutes) + '後轉' + (s.trans.next === 'peak' ? '峰' : '谷') + (s.at ? '（' + s.at + '）' : '')
            : ''
        ...
        if (loading) {
          ...
          body = React.createElement('div', { className: 'dpk-body' },
            ...
            React.createElement('div', { className: 'dpk-note' },
              'DeepSeek：' + PROVIDERS[0].note + '。Ollama：' + PROVIDERS[1].note + '。峰谷時段依兩家官方定價頁（皆以 UTC 定義）。'),
            React.createElement('div', { className: 'dpk-meta' }, '峰谷判定以官方 UTC 為準；顯示時區：' + (cfg.timezone || '本機時間')),
          )
        }

        // ---- Ollama 餘額/用量區塊 ----
        // 配額變化慢，抓取有快取；這裡顯示資料時間與目前的查詢間隔。
        const usageMetaText = (usageFetchedAt === null ? '' : '資料時間 ' + new Date(usageFetchedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + '；')
          + (usageCacheMinutes === null || usageCacheMinutes <= 0 ? '每次都即時查詢' : '每 ' + usageCacheMinutes + ' 分鐘最多查一次 Ollama')
        ...
        if (usageLoading) {
          ...
          const allow = (typeof usage.allowance === 'number' && usage.allowance > 0) ? usage.allowance : null
          const used = (allow !== null && typeof usage.usedDollars === 'number') ? usage.usedDollars : null
          const pct = frac === null ? '?' : (frac * 100).toFixed(1) + '%'
          usageSection = React.createElement('div', { className: 'dpk-body' },
            React.createElement('div', { className: 'dpk-reco' },
              used === null
                ? 'Ollama 本月用量：' + pct
                : 'Ollama 本月用量：' + pct + '（$' + used.toFixed(2) + ' / $' + allow + '）',
            ),
            React.createElement('div', { className: 'dpk-meta' },
              used === null
                ? '尚未設定每月額度（美元），所以只顯示 Ollama 回報的百分比；到「設定」填入後才會換算金額。'
                : '每月額度 $' + allow + '；$ 金額是換算值，百分比才是 Ollama 回報的真實用量。',
            ),
            React.createElement('div', { className: 'dpk-meta' }, usageMetaText),
          )
        }

        const settingsPanel = React.createElement('div', { className: 'dpk-settings' },
          React.createElement('label', { className: 'dpk-field' },
            React.createElement('span', null, '顯示時區（IANA，如 Asia/Taipei，留空=本機；只影響顯示，判定依官方 UTC）'),
            React.createElement('input', { type: 'text', value: tzDraft, placeholder: 'Asia/Taipei', onChange: (e) => setTzDraft(e.target.value) }),
          ),
          React.createElement('label', { className: 'dpk-field' },
            React.createElement('span', null, 'Ollama 每月額度（美元；留空＝不換算，只顯示百分比）'),
            React.createElement('input', { type: 'number', min: '1', step: '1', value: allowanceDraft, placeholder: '例如 20', onChange: (e) => setAllowanceDraft(e.target.value) }),
          ),
          React.createElement('label', { className: 'dpk-field' },
            React.createElement('span', null, '用量查詢間隔（分鐘；0＝每次都查）'),
            React.createElement('input', { type: 'number', min: '0', step: '1', value: cacheDraft, placeholder: '10', onChange: (e) => setCacheDraft(e.target.value) }),
          ),
        ...
        const cardContent = [
          ...
          React.createElement('div', { className: 'dpk-actions' },
            ...
            React.createElement('button', { type: 'button', className: 'dpk-btn', onClick: () => { refresh(); refreshUsage(true) }, disabled: loading || usageLoading }, '重新整理'),
          ),
        ]
        ...
        // 卡片自適應：跟隨膠囊位置、用實測高度夾在視窗內（視窗縮放會即時重算）
        const vw = vp.w
        const vh = vp.h
        const margin = 12
        const cardW = 300
        ...
        return React.createElement(React.Fragment, null,
          open ? React.createElement('div', { ref: cardRef, className: 'dpk-card', style: { position: 'fixed', left: cardLeft + 'px', bottom: cardBottomCss + 'px', marginBottom: 0 } },
            ...cardContent
          ) : null,
